...
$CurrFile='SelectMeetingProtocol.php';
$Frm='MeetingProtocol';
$Fields=array('MId','LineNo'
        ,'No','Description');
$enFields= array();
...
if ($SelId== '9') { 
  ...
  $queryCNT = "select COUNT(*) \"CNT\" FROM \"MeetingProtocol\" ".
              "$WHS ";
  ...
  echo ('<br><b>'.GetStr($pdo, 'MeetingProtocol').' '.
  ...
  //echo ('<hr><br><form method=post action="MeetingProtocolCard.php">'.
...
while ($dp = $STH->fetch(PDO::FETCH_ASSOC)) {
  ...
  foreach ( $Fields as $Fld) {
    if ($Fld=='LineNo') {
      echo("<td align=left><a href='MeetingProtocolCard.php?MId={$dp['MId']}&LineNo={$dp['LineNo']}'>{$dp[$Fld]}</a></td>");
    }
    else 
    if ($enFields[$Fld]!=''){
