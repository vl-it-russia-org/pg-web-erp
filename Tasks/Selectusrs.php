...
$CurrFile='Selectusrs.php';
$Frm='usrs';
$Fields=array('usr_id','usr_pwd'
        ,'description','admin','email'
        ,'phone','passwd_duedate','new_passwd'
...
if ($SelId== '24') { 
  ...
  echo ("<script>".
    "function SetSelect( Val, ValFirstName, ValLastName, ValPatronymicName__c ) { 
       OW=window.opener;
       ...
       elem1 = OW.document.getElementById('FirstName');
       if (elem1) { 
         elem1.value=ValFirstName;
       }
      
       elem1 = OW.document.getElementById('LastName');
       if (elem1) { 
         elem1.value=ValLastName;
       }
      
       elem1 = OW.document.getElementById('MidName');
       if (elem1) { 
         elem1.value=ValPatronymicName__c;
       }
  ...
  //echo ('<hr><br><form method=post action="usrsCard.php">'.
...
while ($dp = $STH->fetch(PDO::FETCH_ASSOC)) {
  ...
  if ($SelId=='24'){
    $Res="\"".addslashes($dp['email'])."\"";
    $Res.=",\"".addslashes($dp['FirstName'])."\"";
      $Res.=",\"".addslashes($dp['LastName'])."\"";
      $Res.=",\"".addslashes($dp['PatronymicName__c'])."\"";
  }
  ...
  foreach ( $Fields as $Fld) {
    if ($Fld=='usr_id') {
      echo("<td align=left><a href='usrsCard.php?usr_id={$dp['usr_id']}'>{$dp[$Fld]}</a></td>");
    }
    else 
    if ($enFields[$Fld]!=''){
