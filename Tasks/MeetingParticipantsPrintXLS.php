...
$objPHPExcel->getProperties()->setCreator("Vladislav Levitskiy")
             ...
             ->setTitle("AccPhp MeetingParticipants")
             ->setSubject("MeetingParticipants")
             ->setDescription("VDL PHP+PDO+PostgreSQL")
             ->setKeywords("AccPhp;MeetingParticipants")
             ->setCategory("AccPhp;MeetingParticipants");
...
$aSheet->setCellValue([$col, $row], GetStr($pdo, 'MeetingParticipants').
      ' '.  GetStr($pdo, 'List'));
...
while ($dp = $STH->fetch(PDO::FETCH_ASSOC)) {
  ...
  $Fld='PartType';
  $aSheet->setCellValue([$col, $row], GetEnum($pdo, 'PartType', $dp[$Fld]));
  $col++;
...
header('Content-Disposition: attachment;filename=Xls-MeetingParticipants'.$add_str.'.xlsx');
