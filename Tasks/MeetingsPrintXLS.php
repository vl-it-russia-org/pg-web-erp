...
$objPHPExcel->getProperties()->setCreator("Vladislav Levitskiy")
             ...
             ->setTitle("AccPhp Meetings")
             ->setSubject("Meetings")
             ->setDescription("VDL PHP+PDO+PostgreSQL")
             ->setKeywords("AccPhp;Meetings")
             ->setCategory("AccPhp;Meetings");
...
$aSheet->setCellValue([$col, $row], GetStr($pdo, 'Meetings').
      ' '.  GetStr($pdo, 'List'));
...
header('Content-Disposition: attachment;filename=Xls-Meetings'.$add_str.'.xlsx');
