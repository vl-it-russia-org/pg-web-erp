...
$CurrFile='SelectToDo_Line.php';
$Frm='ToDo_Line';
$Fields=array('Id','ParentId'
        ,'ToDoCode','Description','DateBeg'
        ,'DateEnd','Status');
...
if ($SelId== '25') { 
  ...
  //echo ('<hr><br><form method=post action="ToDo_LineCard.php">'.
...
while ($dp = $STH->fetch(PDO::FETCH_ASSOC)) {
  ...
  foreach ( $Fields as $Fld) {
    if ($Fld=='Id') {
      echo("<td align=left><a href='ToDo_LineCard.php?Id={$dp['Id']}'>{$dp[$Fld]}</a></td>");
    }
    else 
    if ($enFields[$Fld]!=''){
