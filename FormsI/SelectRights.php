...
$CurrFile='SelectRights.php';
$Frm='Rights';
$Fields=array('RightType','RightDescription'
        ,'HelpLink','NeedLocation','EnumRight'
        ,'HaveTable','TabName','FieldName'
        ,'FldDescription');
$enFields= array();
...
if ($SelId== '22') { 
  ...
  $queryCNT = "select COUNT(*) \"CNT\" FROM \"Rights\" ".
              "$WHS ";
  ...
  //echo ('<hr><br><form method=post action="RightsCard.php">'.
...
while ($dp = $STH->fetch(PDO::FETCH_ASSOC)) {
  ...
  foreach ( $Fields as $Fld) {
    if ($Fld=='RightType') {
      echo("<td align=left><a href='RightsCard.php?RightType={$dp['RightType']}'>{$dp[$Fld]}</a></td>");
    }
    else 
    if ($enFields[$Fld]!=''){
