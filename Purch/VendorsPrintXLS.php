...
$objPHPExcel->getProperties()->setCreator("Vladislav Levitskiy")
             ...
             ->setTitle("AccPhp Vendors")
             ->setSubject("Vendors")
             ->setDescription("VDL PHP+PDO+PostgreSQL")
             ->setKeywords("AccPhp;Vendors")
             ->setCategory("AccPhp;Vendors");
...
$aSheet->setCellValue([$col, $row], GetStr($pdo, 'Vendors').
      ' '.  GetStr($pdo, 'List'));
...
while ($dp = $STH->fetch(PDO::FETCH_ASSOC)) {
  ...
  $Fld='VendorType';
  $aSheet->setCellValue([$col, $row], GetEnum($pdo, 'VendorType', $dp[$Fld]));
  $col++;
  ...
  $Fld='Status';
  $aSheet->setCellValue([$col, $row], GetEnum($pdo, 'StatusNUZ', $dp[$Fld]));
  $col++;
  ...
  $aSheet->setCellValue([$col, $row], GetEnum($pdo, 'GeneralBusinessGroup', $dp[$Fld]));
  $col++;

  $Fld='TaxBusinessGroup';
  $aSheet->setCellValue([$col, $row], GetEnum($pdo, 'TaxBusinessGroup', $dp[$Fld]));
  $col++;
...
header('Content-Disposition: attachment;filename=Xls-Vendors'.$add_str.'.xlsx');
