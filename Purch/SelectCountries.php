...
$CurrFile='SelectCountries.php';
$Frm='Countries';
$Fields=array('Code2','Code3'
        ,'DigCode','CountryName');
$enFields= array();
...
if ($SelId== '20') { 
  ...
  $queryCNT = "select COUNT(*) \"CNT\" FROM \"Countries\" ".
              "$WHS ";
  ...
  echo ('<br><b>'.GetStr($pdo, 'Countries').' '.
        GetStr($pdo, 'List').
  ...
  //echo ('<hr><br><form method=post action="CountriesCard.php">'.
...
while ($dp = $STH->fetch(PDO::FETCH_ASSOC)) {
  ...
  foreach ( $Fields as $Fld) {
    if ($Fld=='Code2') {
      echo("<td align=left><a href='CountriesCard.php?Code2={$dp['Code2']}'>{$dp[$Fld]}</a></td>");
    }
    else 
    if ($enFields[$Fld]!=''){
