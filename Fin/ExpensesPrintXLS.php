...
$objPHPExcel->getProperties()->setCreator("Vladislav Levitskiy")
             ...
             ->setTitle("AccPhp Expenses")
             ->setSubject("Expenses")
             ->setDescription("VDL PHP+PDO+PostgreSQL")
             ->setKeywords("AccPhp;Expenses")
             ->setCategory("AccPhp;Expenses");
...
$aSheet->setCellValue([$col, $row], GetStr($pdo, 'Expenses').
      ' '.  GetStr($pdo, 'List'));
...
while ($dp = $STH->fetch(PDO::FETCH_ASSOC)) {
  ...
  $Fld='FinDiv';
  $aSheet->setCellValue([$col, $row], GetEnum($pdo, 'ExpensesDiv', $dp[$Fld]));
  $col++;
  $row++;
}
...
header('Content-Disposition: attachment;filename=Xls-Expenses'.$add_str.'.xlsx');
